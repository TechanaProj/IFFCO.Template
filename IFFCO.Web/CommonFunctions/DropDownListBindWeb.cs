...
namespace IFFCO.NERRS.Web.CommonFunctions
{
    public class DropDownListBindWeb : DropDownListBind
    {
        ...
        {
            string sqlquery = @"SELECT  A.QUARTER_CATEGORY, A.QUARTER_NO, A.ALLOTMENT_NO,CAST(A.PERSONAL_NO AS VARCHAR(20)) AS Identifier,B.EMP_NAME AS Name, CASE   WHEN A.PERSONAL_NO = '999999' THEN 'Non-Emp'  WHEN B.EMP_NAME IS NOT NULL THEN 'Employee'   ELSE 'Non-Emp'  END AS Type  FROM  F_ALLOTMENT_RENT_DTLS A LEFT JOIN V_EB_EMPLOYEE_COMPLETE_DTLS B ON A.PERSONAL_NO = B.PERSONAL_NO WHERE A.Status = 'A' AND A.PERSONAL_NO IS NOT NULL UNION SELECT A.QUARTER_CATEGORY, A.QUARTER_NO, A.ALLOTMENT_NO, A.VENDOR_CODE AS Identifier,C.VENDOR_NAME AS Name, 'Vendor' AS Type  FROM F_ALLOTMENT_RENT_DTLS A JOIN M_VENDOR_MSTS C ON A.VENDOR_CODE = C.VENDOR_CODE WHERE A.Status = 'A' ORDER BY ALLOTMENT_NO ASC";
        ...
        {
            string sqlquery = @"SELECT  A.QUARTER_CATEGORY, A.QUARTER_NO, A.ALLOTMENT_NO,CAST(A.PERSONAL_NO AS VARCHAR(20)) AS Identifier, B.EMP_NAME AS Name,CASE WHEN A.PERSONAL_NO = '999999' THEN 'Non-Emp'  WHEN B.EMP_NAME IS NOT NULL THEN 'Employee'   ELSE 'Non-Emp' END AS Type  FROM  F_INT_COMPUTE A  LEFT JOIN V_EB_EMPLOYEE_COMPLETE_DTLS B ON A.PERSONAL_NO = B.PERSONAL_NO WHERE A.PERSONAL_NO is not null UNION SELECT A.QUARTER_CATEGORY, A.QUARTER_NO, A.ALLOTMENT_NO, A.VENDOR_CODE AS Identifier,C.VENDOR_NAME AS Name, 'Vendor' AS Type FROM F_INT_COMPUTE A JOIN M_VENDOR_MSTS C ON A.VENDOR_CODE = C.VENDOR_CODE  ORDER BY ALLOTMENT_NO ASC";
